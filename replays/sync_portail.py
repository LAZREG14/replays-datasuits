...
2. REPLAYS  (board monday « Replays CV »)
   - Statut « À traiter » → téléchargement du lien SharePoint, compression 720p,
     ...
     Quelques replays par nuit (MAX_REPLAYS), pour rester dans les temps.
   - Statut « Archivé » (posé à la main) ou date de purge dépassée
     → dépublication + suppression du fichier dans R2.

3. Chaque appel à Supabase sert aussi de « ping » : le projet gratuit ne se
   met jamais en pause.
...
def sync_replays():
    ...
    log(f"   {len(a_traiter)} replay(s) à traiter, {MAX_REPLAYS} max ce soir")
